...
This is the centralized authentication service frontend for TechWiki. It provides a single authentication endpoint at `auth.techwiki.co.uk` for all TechWiki applications.
...
- **Session management**: Secure session cookies shared across `.techwiki.co.uk`
...
1. User visits app (app.techwiki.co.uk)
2. If not authenticated, redirect to auth.techwiki.co.uk/login?next=app.techwiki.co.uk
3. User authenticates with passkey or password
4. On success, redirect back to the `next` URL
5. Session cookie is set on `.techwiki.co.uk` domain
