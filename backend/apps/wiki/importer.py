...
class MediaWikiConverter:
    ...
    def __init__(self) -> None:
        self.inferred_categories: list[str] = []
        self.generated_redirects: list[dict[str, str]] = []

    def convert(self, wiki_text: str, title: str = "") -> dict[str, Any]:
        """
    ...
    def _convert_links(self, text: str) -> str:
        ...
        # Internal links [[Page]]
        text = re.sub(
            r"\[\[([^\]]+)\]\]", lambda m: f"[{m.group(1)}](/{slugify(m.group(1))})", text
        )
    ...
    def _convert_templates(self, text: str) -> str:
        ...
        # {{note|...}} -> > **Note:** ...
        text = re.sub(r"\{\{[Nn]ote\|([^}]+)\}\}", r"> **Note:** \1", text)

        # {{warning|...}} -> > ⚠️ **Warning:** ...
        text = re.sub(r"\{\{[Ww]arning\|([^}]+)\}\}", r"> ⚠️ **Warning:** \1", text)

        # {{tip|...}} -> > 💡 **Tip:** ...
        text = re.sub(r"\{\{[Tt]ip\|([^}]+)\}\}", r"> 💡 **Tip:** \1", text)
...
def import_mediawiki_page(
    ...
    if category_slug:
        category, _ = Category.objects.get_or_create(
            slug=category_slug, defaults={"name": category_slug.replace("-", " ").title()}
        )
    ...
    for redirect_data in result["redirects"]:
        redirect, _ = Redirect.objects.get_or_create(
            old_path=redirect_data["from"], defaults={"new_path": redirect_data["to"]}
        )
    ...
    wiki_redirect, _ = Redirect.objects.get_or_create(
        old_path=f"/wiki/{title.replace(' ', '_')}", defaults={"new_path": article.full_url}
    )
    ...
    index_redirect, _ = Redirect.objects.get_or_create(
        old_path=f"/index.php/{title.replace(' ', '_')}", defaults={"new_path": article.full_url}
    )
