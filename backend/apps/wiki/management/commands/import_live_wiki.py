...
CATEGORY_KEYWORDS = {
    "linux": ["linux", "ubuntu", "centos", "debian", "nginx", "apache", "bash", "ssh", "disk", "kernel", "iptables"],
    "windows": ["windows", "microsoft", "outlook", "office", "dpm", "active directory", "group policy", "admx"],
    ...
    def handle(self, *args, **options):
        ...
        for i, page in enumerate(pages, 1):
            ...
            try:
                ...
                if not wiki_text:
                    self.stdout.write(self.style.WARNING(f"  Empty content"))
                    pages_skipped += 1
                    continue

                # Check if it's a redirect
                if wiki_text.strip().upper().startswith("#REDIRECT"):
                    self.stdout.write(f"  Skipping redirect page")
                    pages_skipped += 1
                    continue
            ...
            if dry_run:
                inferred_cat = infer_category(title) if not default_category else default_category
                self.stdout.write(self.style.SUCCESS(f"  Would import: {title} -> [{inferred_cat}]"))
                pages_imported += 1
                time.sleep(delay)
                continue
            ...
            try:
                ...
                Redirect.objects.get_or_create(
                    old_path=f"/{article.slug}",
                    defaults={"new_path": article.full_url}
                )

                categories_str = ", ".join(all_categories)
                action = "Created" if result["created"] else "Updated"
                self.stdout.write(self.style.SUCCESS(f"  {action}: {result['article'].title} [{categories_str}]"))
                pages_imported += 1
